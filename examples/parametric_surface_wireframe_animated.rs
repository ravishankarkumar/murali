...
fn main() -> anyhow::Result<()> {
    ...
    let title_id = scene.add_tattva(
        Label::new("Animated Wireframe Surface", 0.36)
    ...
    let wavy_animated = ParametricSurface::new(
        (-PI, PI),
        (-PI, PI),
        |u, v| {
            let x = u;
    ...
    let surface_id = scene.add_tattva(wavy_animated, Vec3::new(0.0, 0.0, 0.0));

    // Subtitle
    scene.add_tattva(
        Label::new("Grid lines drawn progressively: horizontal → vertical", 0.18)
