...
fn main() -> anyhow::Result<()> {
    ...
    let eq_from_id = add_tattva(
        &mut scene,
        EquationLayout::new(
            vec![
                EquationPart::new("y").with_key("lhs"),
                EquationPart::new("=").with_key("eq").with_color(Vec4::new(0.94, 0.88, 0.46, 1.0)),
                EquationPart::new("Wx").with_key("expr").with_color(Vec4::new(0.37, 0.80, 0.97, 1.0)),
                EquationPart::new("+").with_key("plus"),
                EquationPart::new("b").with_key("bias").with_color(Vec4::new(0.98, 0.58, 0.34, 1.0)),
            ],
            0.34,
        ),
    ...
    let eq_to_id = add_tattva(
        &mut scene,
        EquationLayout::new(
            vec![
                EquationPart::new("softmax").with_key("fn").with_color(Vec4::new(0.71, 0.56, 0.98, 1.0)),
                EquationPart::new("("),
                EquationPart::new("Wx").with_key("expr").with_color(Vec4::new(0.37, 0.80, 0.97, 1.0)),
                EquationPart::new("+").with_key("plus"),
                EquationPart::new("b").with_key("bias").with_color(Vec4::new(0.98, 0.58, 0.34, 1.0)),
    ...
    timeline
        ...
        .for_duration(1.0)
        .ease(Ease::OutQuad)
        .matrix_step_cells(vec![(0, 1), (1, 2), (2, 0)], Vec4::new(0.98, 0.66, 0.22, 0.60), 0.28)
        .spawn();
