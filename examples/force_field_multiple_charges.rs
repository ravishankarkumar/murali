...
fn main() -> anyhow::Result<()> {
    ...
    let positive_charge = Circle::new(particle_radius, 24, Vec4::new(1.0, 0.2, 0.2, 1.0))
        .with_stroke(0.04, Vec4::new(1.0, 0.5, 0.5, 1.0));
    let positive_id = scene.add_tattva(positive_charge, Vec3::new(-2.0, 0.0, 0.0));

    // Negative charge (blue) - orbiting opposite direction
    let negative_charge = Circle::new(particle_radius, 24, Vec4::new(0.2, 0.4, 1.0, 1.0))
        .with_stroke(0.04, Vec4::new(0.5, 0.6, 1.0, 1.0));
    let negative_id = scene.add_tattva(negative_charge, Vec3::new(2.0, 0.0, 0.0));
    ...
    scene.add_updater(positive_id, move |scene, pid, _dt| {
        ...
        scene.set_position(pid, Vec2::new(x, y));
    });
    ...
    scene.add_updater(negative_id, move |scene, nid, _dt| {
        ...
        scene.set_position(nid, Vec2::new(x, y));
    });
            ...
            if let (Some(pos_pos), Some(neg_pos)) = (pos_charge_pos, neg_charge_pos) {
                ...
                if force_magnitude > 0.01 {
                    ...
                    if let Some(vector_tattva) = scene.get_tattva_any_mut(vid) {
                        ...
                        props.scale = vec3(1.0, force_magnitude * 0.35, 1.0);
                    }
                }
            }
        });
