...
use murali::frontend::collection::graph::stream_lines::{StreamLines, line_start_points, circle_start_points};
...
fn main() -> anyhow::Result<()> {
    ...
    let title_id = scene.add_tattva(
        Label::new("StreamLines Showcase", 0.36)
    ...
    scene.add_tattva(
        Label::new("Radial Source", 0.16).with_color(Vec4::new(0.8, 0.8, 0.8, 1.0)),
        Vec3::new(-4.5, 2.8, 0.0),
    );
    ...
    .with_color(Vec4::new(1.0, 0.4, 0.4, 0.8))
    .with_thickness(0.025)
    .with_step_size(0.08)
    .with_max_steps(50)
    .with_bounds(Vec2::new(-2.0, 0.5), Vec2::new(2.0, 2.5));

    scene.add_tattva(radial_stream, Vec3::new(-4.5, 0.0, 0.0));

    // 2. Circular/Vortex flow
    scene.add_tattva(
        Label::new("Vortex Flow", 0.16).with_color(Vec4::new(0.8, 0.8, 0.8, 1.0)),
    ...
    .with_color(Vec4::new(0.4, 1.0, 0.4, 0.8))
    .with_thickness(0.025)
    .with_step_size(0.08)
    .with_max_steps(100)
    .with_bounds(Vec2::new(-2.0, 0.5), Vec2::new(2.0, 2.5));

    scene.add_tattva(vortex_stream, Vec3::new(0.0, 0.0, 0.0));

    // 3. Saddle point flow
    scene.add_tattva(
        Label::new("Saddle Point", 0.16).with_color(Vec4::new(0.8, 0.8, 0.8, 1.0)),
        Vec3::new(4.5, 2.8, 0.0),
    );
    ...
    .with_color(Vec4::new(0.4, 0.4, 1.0, 0.8))
    .with_thickness(0.025)
    .with_step_size(0.08)
    .with_max_steps(50)
    .with_bounds(Vec2::new(-2.0, 0.5), Vec2::new(2.0, 2.5));

    scene.add_tattva(saddle_stream, Vec3::new(4.5, 0.0, 0.0));

    // 4. Uniform flow with obstacle (flow around)
    scene.add_tattva(
        Label::new("Flow Around Obstacle", 0.16).with_color(Vec4::new(0.8, 0.8, 0.8, 1.0)),
        Vec3::new(-4.5, -0.8, 0.0),
    );

    let obstacle_starts = line_start_points(Vec2::new(-1.8, 0.5), Vec2::new(-1.8, 2.0), 6);
    let obstacle_stream = StreamLines::new(
        obstacle_starts,
        |pos: Vec2| {
    ...
    .with_color(Vec4::new(1.0, 0.7, 0.4, 0.8))
    .with_thickness(0.025)
    .with_step_size(0.06)
    .with_max_steps(80)
    .with_bounds(Vec2::new(-2.0, -2.5), Vec2::new(2.0, -0.5));

    scene.add_tattva(obstacle_stream, Vec3::new(-4.5, 0.0, 0.0));

    // 5. Dipole field
    scene.add_tattva(
        Label::new("Dipole Field", 0.16).with_color(Vec4::new(0.8, 0.8, 0.8, 1.0)),
        Vec3::new(0.0, -0.8, 0.0),
    );

    let dipole_starts = line_start_points(Vec2::new(-1.5, 2.0), Vec2::new(1.5, 2.0), 7);
    let dipole_stream = StreamLines::new(
        dipole_starts,
        |pos: Vec2| {
    ...
    .with_color(Vec4::new(0.7, 0.4, 1.0, 0.8))
    .with_thickness(0.025)
    .with_step_size(0.06)
    .with_max_steps(100)
    .with_bounds(Vec2::new(-2.0, -2.5), Vec2::new(2.0, -0.5));

    scene.add_tattva(dipole_stream, Vec3::new(0.0, 0.0, 0.0));

    // 6. Magnitude-colored streamlines
    scene.add_tattva(
        Label::new("Magnitude Colored", 0.16).with_color(Vec4::new(0.8, 0.8, 0.8, 1.0)),
        Vec3::new(4.5, -0.8, 0.0),
    );

    let colored_starts = StreamLines::from_grid(
        (-1.8, 1.8),
        (0.6, 2.4),
        3,
        4,
    ...
    scene.add_tattva(colored_starts, Vec3::new(4.5, 0.0, 0.0));
